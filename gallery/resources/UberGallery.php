...
var $nav;
    ...
    public function printDebug()
    {
        ...
        global $currentpage;
        global $totalPages;
        $newCurrentIndex = new UberGallery();
        $newCurrentIndex->_getPaginatorArray($currentpage,$totalPages);
        if ($this->_config['debugging']) {    //TODO: sajátDebugging
            print_r($nav);
            print_r($_GET);
            print_r($newCurrentIndex);
        }
    }
    public function __construct() {
        ...
        if (file_exists($configPath)) {
            ...
            $this->setSajatDebug($config['advanced_settings']['sajat_debug']);
     ...
     * @access public
     */
    public function setThumbSize($width = 150 , $height = 150) {    // SIZE DOES MATTER!
    ...
    public function setSajatDebug($bool = false) {
        $this->_config['debugging'] = $bool;
    ...
    private function _getPaginatorArray($currentPage, $totalPages) {
        ...
        global $nav;  //saját változó
        $nav = new paginationParams();
        $nav->pageParamsSet();
        ...
        if ($firstPage > 1) {
            $paginatorArray[] = array(
                'text'  => '...',
                'class' => 'more',
                'href'  => '?page=' . ($currentPage - $range - 1).$nav
            );
        }
        ...
        // Set next overflow
        //Következő oldal ( --> HREF)
        if ($lastPage < $totalPages) {
            $paginatorArray[] = array(
                'text'  => '...',
                'class' => 'more',
                'href'  => '?page=' . ($currentPage + $range + 1).$nav.'teszt'
            );
        }
        ...
        return $currentPage;   //TODO: Am I wrong?
